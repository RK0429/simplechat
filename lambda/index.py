...
def lambda_handler(event, context):
    try:
        ...
        print(
            "Calling Bedrock invoke_model API with payload:",
        ...
        # 応答の検証
        if (
            not response_body.get("output")
            or not response_body["output"].get("message")
            or not response_body["output"]["message"].get("content")
        ):
